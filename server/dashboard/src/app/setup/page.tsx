...
const SUPPORTED_PROVIDERS_URL = "/docs";
...
export default function SetupPage() {
  ...
  const { t } = useI18n();
  const localizedSteps = STEPS.map((label) => t(label));
  const localizedStepTitles = STEP_TITLES.map((title) => t(title));
  const localizedUseCasePresets = USE_CASE_PRESETS.map((value) => ({
    value,
    label: t(value),
  }));
          ...
          <CardContent className="p-6 space-y-4">
            <div className="space-y-1">
              <h2 className="text-base font-semibold font-fustat">
                {localizedStepTitles[step]}
              </h2>
              {isPrefillingConfig && step === 1 && (
                <p className="text-xs text-onSurface-default-tertiary">
                  {t("Checking server configuration...")}
                </p>
              )}
            </div>
            {error && (
            ...
            {step === 0 && (
              <form onSubmit={handleStep1} className="space-y-4">
                ...
                <Button
                  ...
                  className="w-full"
                >
                  {t(isLoading ? "Creating..." : "Create Admin Account")}
                </Button>
              </form>
            )}
            ...
            {step === 1 && (
              <form onSubmit={handleStep2} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <Label htmlFor="setup-llm-provider">
                      {t("LLM Provider")}
                    </Label>
                    <Select
                      value={llmProvider}
                  ...
                  <div className="space-y-1">
                    <Label htmlFor="setup-llm-model">{t("Model")}</Label>
                    <Input
                      id="setup-llm-model"
                      value={llmModel}
                ...
                <div className="space-y-1">
                  <Label htmlFor="setup-llm-api-key">{t("LLM API Key")}</Label>
                  <Input
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    <Label htmlFor="setup-embedder-model">{t("Model")}</Label>
                    <Input
                ...
                <Button
                  ...
                  className="w-full"
                >
                  {t(isLoading ? "Saving..." : "Save & Continue")}
                </Button>
              </form>
            )}
            ...
            {step === 2 && !apiKey && (
              <form onSubmit={handleStep3} className="space-y-4">
                <div className="space-y-1">
                  <Label htmlFor="setup-key-label">
                    {t("Label for this key")}
                  </Label>
                  <Input
                    id="setup-key-label"
                    value={keyLabel}
                    onChange={(e) => setKeyLabel(e.target.value)}
                    placeholder={t("My First Key")}
                  />
                </div>
                <Button type="submit" disabled={isLoading} className="w-full">
                  {t(isLoading ? "Generating..." : "Generate API Key")}
                </Button>
              </form>
            )}
            ...
            {step === 2 && apiKey && (
              <form onSubmit={handleContinueToUseCase} className="space-y-4">
                <div className="space-y-1">
                  <Label htmlFor="setup-api-key">{t("Your API Key")}</Label>
                  ...
                  <p className="text-xs text-onSurface-danger-primary">
                    {t("Save this key. You will not see it again.")}
                  </p>
                </div>
            ...
            {step === 3 && (
              <div className="space-y-4">
                <div className="space-y-1">
                  <Label htmlFor="setup-use-case">
                    {t("Describe your use case")}
                  </Label>
                  <textarea
                    id="setup-use-case"
                    value={useCase}
                ...
                <div className="flex flex-wrap gap-2">
                  {localizedUseCasePresets.map(({ value, label }) => (
                    <Button
                      key={value}
                      type="button"
                      variant="outline"
                ...
                {!testSuccess ? (
                  <>
                    <Button
                      ...
                      className="w-full"
                    >
                      {t(isLoading ? "Testing..." : "Run Test")}
                    </Button>
                    {error && (
                      <p className="text-xs text-onSurface-default-tertiary">
                        {t("Provider credentials or model wrong? Fix them in")}{" "}
                        <a

    ...
    scope = _build_session_scope(
        {
            ...
            "app_id": "dashboard",
            "project_id": "project-a.__playground__",
        }
    )

    assert scope == "app_id=dashboard&project_id=project-a.__playground__&user_id=playground-user"


def test_project_metadata_is_promoted_to_conversation_filters():
    metadata, filters = _build_filters_and_metadata(
        user_id="playground-user",
        app_id="dashboard",
        input_metadata={"project_id": "project-a.__playground__"},
    )
...
def test_inference_context_lookup_is_project_scoped():
    with patch.object(Memory, "__init__", return_value=None):
        memory = Memory()
    ...
    result = memory._add_to_vector_store(
        ...
        metadata={"project_id": "project-a.__playground__"},
        filters=filters,
        infer=True,
    )
    ...
    memory.db.get_last_messages.assert_called_once_with(
        "project_id=project-a.__playground__&user_id=playground-user",
        limit=10,
    )
    memory.vector_store.search.assert_called_once_with(
        query="user: Private project context\n",
        vectors=[0.1, 0.2, 0.3],
        top_k=10,
        filters={
            "project_id": "project-a.__playground__",
...
def test_history_write_records_project_scope():
    with patch.object(Memory, "__init__", return_value=None):
        memory = Memory()
    memory.embedding_model = MagicMock()
    memory.vector_store = MagicMock()
    ...
    memory._create_memory(
        "Scoped memory",
        {"Scoped memory": [0.1, 0.2, 0.3]},
        {"project_id": "project-a", "user_id": "user-a"},
    )

    assert memory.db.add_history.call_args.kwargs["project_id"] == "project-a"
...
def test_delete_all_combines_app_and_project_filters():
    with patch.object(Memory, "__init__", return_value=None):
        memory = Memory()
    memory.vector_store = MagicMock()
    ...
    with (
        ...
        result = memory.delete_all(app_id="app-a", project_id="project-a")

    assert result == {"message": "Memories deleted successfully!"}
    memory.vector_store.list.assert_called_once_with(filters={"app_id": "app-a", "project_id": "project-a"})
...
@patch("mem0.utils.factory.VectorStoreFactory.create")
@patch("mem0.utils.factory.LlmFactory.create")
async def test_async_memory_reset_clears_messages_table(
...
def test_read_apis_surface_attributed_to(mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory):
    ...
    assert got["categories"] == ["task_learning"]
    assert "attributed_to" not in (got.get("metadata") or {})
    assert "categories" not in (got.get("metadata") or {})
    ...
    assert listed[0]["categories"] == ["task_learning"]
    assert "attributed_to" not in (listed[0].get("metadata") or {})
    assert "categories" not in (listed[0].get("metadata") or {})
    ...
    assert searched[0]["categories"] == ["task_learning"]
    assert "attributed_to" not in (searched[0].get("metadata") or {})
    assert "categories" not in (searched[0].get("metadata") or {})
...
@patch("mem0.utils.factory.LlmFactory.create")
@patch("mem0.memory.storage.SQLiteManager")
async def test_async_read_apis_surface_attributed_to(
    mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory
):
...
def test_add_infer_with_malformed_llm_facts(mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory):
    ...
    """
    mock_embedder = MagicMock()
    mock_embedder.embed.side_effect = lambda text, action: (
        (_ for _ in ()).throw(AttributeError("'dict' object has no attribute 'replace'"))
    ...
    # The V3 extraction contract requires a ``memory`` list. A legacy/malformed
    # ``facts`` payload must be retryable instead of being silently accepted.
    with pytest.raises(LLMError, match="missing the 'memory' field"):
...
@patch("mem0.utils.factory.LlmFactory.create")
@patch("mem0.memory.storage.SQLiteManager")
def test_update_propagates_vector_store_failure(
    mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory
):
...
@patch("mem0.utils.factory.LlmFactory.create")
@patch("mem0.memory.storage.SQLiteManager")
async def test_async_update_propagates_vector_store_failure(
    mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory
):
...
def test_add_infer_true_caches_embedding_on_llm_rewrite(
    ...
    # V3 pipeline: the memory text is embedded exactly once in Phase 3.
    assert embedder.embed.call_count == 1
    assert embedder.embed_batch.call_count == 1
    memory_batches = [
        call for call in embedder.embed_batch.call_args_list if call.args[0] == ["The user enjoys Python"]
    ]
    assert len(memory_batches) == 1
    mock_vector_store.insert.assert_called_once()
...
@patch("mem0.utils.factory.LlmFactory.create")
@patch("mem0.memory.storage.SQLiteManager")
def test_add_infer_true_persists_allowed_categories(
    mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory
):
    embedder = MagicMock()
    embedder.embed.return_value = [0.1, 0.2, 0.3]
    ...
    mock_llm.generate_response.return_value = json.dumps(
        {"memory": [{"text": "The user prefers concise answers", "categories": ["User Preference", "ignored"]}]}
    )
    mock_llm_factory.return_value = mock_llm
    mock_sqlite.return_value = MagicMock()
    ...
    result = memory.add(
        ...
        user_id="test_user",
        metadata={"_allowed_categories": ["user_preference", "task_learning"]},
        infer=True,
    )

    payloads = mock_vector_store.insert.call_args.kwargs["payloads"]
    assert payloads[0]["categories"] == ["user_preference"]
    assert "_allowed_categories" not in payloads[0]
    assert result["results"][0]["categories"] == ["user_preference"]
...
def test_update_infer_true_caches_embedding_on_llm_rewrite(
    ...
    assert embedder.embed.call_count == 1
    assert embedder.embed_batch.call_count == 1
    memory_batches = [call for call in embedder.embed_batch.call_args_list if call.args[0] == ["The user loves Python"]]
    assert len(memory_batches) == 1
    mock_vector_store.insert.assert_called_once()
...
@patch("mem0.utils.factory.LlmFactory.create")
@patch("mem0.memory.main.SQLiteManager")
async def test_async_delete_all_continues_on_partial_failure(
    mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory
):
...
class TestPreserveCustomMetadata:
    ...
    @patch("mem0.utils.factory.LlmFactory.create")
    @patch("mem0.memory.storage.SQLiteManager")
    def test_update_recomputes_text_lemmatized_after_metadata_merge(
        self, mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory
    ):
        mock_embedder_factory.return_value = MagicMock()
        mock_vector_store = MagicMock()
        ...
        existing_payload = {
            "data": "The verification color is teal",
            "hash": "old-hash",
            "text_lemmatized": "verification color teal",
        ...
        memory = Memory(MemoryConfig())
        new_data = "The verification color is blue-green"

        with patch(
            "mem0.memory.main.lemmatize_for_bm25",
            return_value="verification color blue green",
        ) as normalize:
            memory._update_memory(
                "mem-1",
                new_data,
                {new_data: [0.1, 0.2, 0.3]},
                metadata={"text_lemmatized": "caller supplied stale value"},
            )

        payload = mock_vector_store.update.call_args.kwargs["payload"]
        normalize.assert_called_once_with(new_data)
        assert payload["text_lemmatized"] == "verification color blue green"
        assert payload["data"] == new_data
    ...
    @patch("mem0.utils.factory.LlmFactory.create")
    @patch("mem0.memory.storage.SQLiteManager")
    async def test_async_update_recomputes_text_lemmatized_after_metadata_merge(
        self, mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory
    ):
        mock_embedder_factory.return_value = MagicMock()
        mock_vector_store = MagicMock()
    ...
    @patch("mem0.utils.factory.LlmFactory.create")
    @patch("mem0.memory.storage.SQLiteManager")
    def test_update_preserves_custom_metadata(
        self, mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory
    ):
        mock_embedder_factory.return_value = MagicMock()
        mock_vector_store = MagicMock()
        ...
        memory._update_memory(
            "mem-1", "I love playing tennis and swimming", {"I love playing tennis and swimming": [0.1, 0.2, 0.3]}
        )
    ...
    @patch("mem0.utils.factory.LlmFactory.create")
    @patch("mem0.memory.storage.SQLiteManager")
    def test_update_with_new_metadata_overrides_existing(
        self, mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory
    ):
        mock_embedder_factory.return_value = MagicMock()
        mock_vector_store = MagicMock()
    ...
    @patch("mem0.utils.factory.LlmFactory.create")
    @patch("mem0.memory.storage.SQLiteManager")
    def test_update_preserves_actor_id_from_original(
        self, mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory
    ):
        mock_embedder_factory.return_value = MagicMock()
        mock_vector_store = MagicMock()
    ...
    @patch("mem0.utils.factory.LlmFactory.create")
    @patch("mem0.memory.storage.SQLiteManager")
    async def test_async_update_preserves_custom_metadata(
        self, mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory
    ):
        mock_embedder_factory.return_value = MagicMock()
        mock_vector_store = MagicMock()
...
class TestAsyncDeleteAllEntityRace:
    ...
    @patch("mem0.utils.factory.LlmFactory.create")
    @patch("mem0.memory.storage.SQLiteManager")
    async def test_async_delete_all_bulk_clears_entity_store(
        self, mock_sqlite, mock_llm_factory, mock_vector_factory, mock_embedder_factory
    ):
        """

...
NEPTUNE_ANALYTICS_TESTS_ENABLED = os.getenv("RUN_TEST_NEPTUNE_ANALYTICS", "").strip().lower() == "true"
GRAPH_ID = os.environ.get("GRAPH_ID", "").strip()
...
@pytest.mark.skipif(
    not RUN_EXTERNAL_PROVIDER_TESTS or not NEPTUNE_ANALYTICS_TESTS_ENABLED or not GRAPH_ID,
    reason=(
        "Neptune Analytics tests require YIQIAO_RUN_EXTERNAL_PROVIDER_TESTS=true, "
        "RUN_TEST_NEPTUNE_ANALYTICS=true, and GRAPH_ID"
    ),
)
class TestNeptuneAnalyticsOperations:
    ...
    def na_instance(self):
        ...
        config = {
            "endpoint": f"neptune-graph://{GRAPH_ID}",
...
class TestNeptuneFilterValidation:
    ...
    def test_where_clause_escapes_values(self):
        clause = NeptuneAnalyticsVector._get_where_clause({"user_id": "it's a test"})
        assert "it\\'s a test" in clause

    def test_where_clause_rejects_dict(self):
        with pytest.raises(ValueError):
            NeptuneAnalyticsVector._get_where_clause({"user_id": {"$ne": ""}})

    def test_node_filter_escapes_values(self):
        clause = NeptuneAnalyticsVector._get_node_filter_clause({"label": "it's"})
        assert "it\\'s" in clause

    def test_node_filter_rejects_dict(self):
        with pytest.raises(ValueError):
            NeptuneAnalyticsVector._get_node_filter_clause({"user_id": {"$ne": ""}})

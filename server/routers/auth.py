...
def register(request: Request, body: RegisterRequest, db: Session = Depends(get_db)):
    """Create the first admin account, or accept a local workspace invite."""
    _require_password_length(body.password)

    email = str(body.email).strip().lower()
    user_count = db.scalar(select(func.count(User.id))) or 0
    workspace = get_json(db, WORKSPACE_KEY, DEFAULT_WORKSPACE_SETTINGS)
    invite = None if user_count == 0 else invited_member(workspace, email)
    if user_count > 0 and invite is None:
        raise HTTPException(status_code=403, detail="Registration is open only for invited workspace members.")
    ...
    if user_count == 0:
        capture_admin_registered(email=email)
    else:
        set_json(db, WORKSPACE_KEY, activate_member(workspace, email))
...
def update_me(
    ...
    if body.email is not None and body.email != user.email:
        email = str(body.email).strip().lower()
        collision = db.scalar(select(User).where(func.lower(User.email) == email, User.id != user.id))
        ...
        set_json(
            db,
            WORKSPACE_KEY,
            replace_member_email(workspace, user.email, email),
            commit=False,
        )
        user.email = email
...
def delete_me(
    ...
    workspace["members"] = [
        member for member in workspace.get("members", []) if str(member.get("email") or "").strip().lower() != email
    ]
    set_json(db, WORKSPACE_KEY, workspace, commit=False)
    db.delete(user)
    db.commit()
...
def change_password(
    ...
    db.execute(
        ...
        .where(RefreshTokenJti.user_id == user.id, RefreshTokenJti.used_at.is_(None))
        .values(used_at=now)
    )
    db.commit()

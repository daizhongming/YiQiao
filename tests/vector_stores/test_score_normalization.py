...
DIMS = 128
RUN_EXTERNAL_PROVIDER_TESTS = os.getenv("YIQIAO_RUN_EXTERNAL_PROVIDER_TESTS", "").strip().lower() == "true"
...
@pytest.mark.skipif(
    not RUN_EXTERNAL_PROVIDER_TESTS or not _pgvector_reachable(),
    reason=(
        "external provider tests are disabled or "
        f"pgvector is not reachable at {PGVECTOR_HOST}:{PGVECTOR_PORT} with user {PGVECTOR_USER}"
    ),
)
class TestPGVector:
...
@pytest.mark.skipif(
    not RUN_EXTERNAL_PROVIDER_TESTS or not _tcp_reachable(REDIS_HOST, REDIS_PORT),
    reason=f"external provider tests are disabled or Redis is not reachable at {REDIS_HOST}:{REDIS_PORT}",
)
class TestRedis:
...
@pytest.mark.skipif(
    not RUN_EXTERNAL_PROVIDER_TESTS or not _tcp_reachable(VALKEY_HOST, VALKEY_PORT),
    reason=f"external provider tests are disabled or Valkey is not reachable at {VALKEY_HOST}:{VALKEY_PORT}",
)
class TestValkey:
...
@pytest.mark.skipif(
    not RUN_EXTERNAL_PROVIDER_TESTS or not _tcp_reachable(MILVUS_HOST, MILVUS_PORT),
    reason=f"external provider tests are disabled or Milvus is not reachable at {MILVUS_HOST}:{MILVUS_PORT}",
)
class TestMilvusL2:
...
@pytest.mark.skipif(
    not RUN_EXTERNAL_PROVIDER_TESTS or not SUPABASE_CONN,
    reason="external provider tests are disabled or SUPABASE_CONN_STRING is not set",
)
class TestSupabase:
...
@pytest.mark.skipif(
    not RUN_EXTERNAL_PROVIDER_TESTS or not S3_BUCKET,
    reason="external provider tests are disabled or S3_VECTORS_BUCKET is not set",
)
class TestS3Vectors:

...
class TestPGVector(unittest.TestCase):
    ...
    def test_insert_psycopg2(self, mock_get_cursor, mock_connection_pool):
        ...
        with patch.dict(
            ...
            mock_psycopg2.sql.SQL.assert_any_call("INSERT INTO {} (id, vector, payload) VALUES %s")
    ...
    def test_search_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        search_calls = [call for call in self.mock_cursor.execute.call_args_list if "SELECT id, vector <=" in str(call)]
    ...
    def test_get_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        get_calls = [
            call for call in self.mock_cursor.execute.call_args_list if "SELECT id, vector, payload" in str(call)
        ]
    ...
    def test_col_info_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        info_calls = [call for call in self.mock_cursor.execute.call_args_list if "SELECT table_name" in str(call)]
    ...
    def test_reset_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        drop_calls = [call for call in self.mock_cursor.execute.call_args_list if "DROP TABLE IF EXISTS" in str(call)]
        create_calls = [
            call for call in self.mock_cursor.execute.call_args_list if "CREATE TABLE IF NOT EXISTS" in str(call)
        ]
    ...
    def test_update_vector_only_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        vector_update_calls = [
            ...
            and "test_collection" in str(call)
            and "SET vector" in str(call)
            and "payload" not in str(call)
        ]
        payload_update_calls = [
            call
            for call in self.mock_cursor.execute.call_args_list
            if "UPDATE" in str(call) and "test_collection" in str(call) and "SET payload" in str(call)
        ]
    ...
    def test_connection_string_with_sslmode_psycopg3(self, mock_connection_pool):
        ...
        mock_connection_pool.assert_called_with(conninfo=expected_conn_string, min_size=1, max_size=4, open=False)
    ...
    def test_create_col_with_diskann_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        diskann_calls = [call for call in self.mock_cursor.execute.call_args_list if "USING diskann" in str(call)]
    ...
    def test_create_col_with_hnsw_psycopg3(self, mock_get_cursor, mock_connection_pool):
        ...
        hnsw_calls = [call for call in self.mock_cursor.execute.call_args_list if "USING hnsw" in str(call)]

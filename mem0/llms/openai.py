...
class OpenAILLM(LLMBase):
    def __init__(self, config: Optional[Union[BaseLlmConfig, OpenAIConfig, Dict]] = None):
        ...
        client_options = {}
        if getattr(self.config, "request_timeout", None) is not None:
            client_options["timeout"] = self.config.request_timeout
        ...
        if os.environ.get("OPENROUTER_API_KEY"):  # Use OpenRouter
            ...
            self.client = OpenAI(api_key=api_key, base_url=base_url, **client_options)
    ...
    def generate_response(
        ...
        **kwargs,
        ...
        normalized_finish_reason = finish_reason.strip().lower() if isinstance(finish_reason, str) else ""
        if normalized_finish_reason in {"length", "max_tokens", "max_output_tokens"}:
            raise LLMError(
                f"LLM response was truncated ({normalized_finish_reason}); "
                f"configured output limit is {self.config.max_tokens} tokens."
            )

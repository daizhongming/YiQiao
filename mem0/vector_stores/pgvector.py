...
class PGVector(VectorStoreBase):
    ...
    def existing_payload_values(
        ...
        *,
        ...
        """Return matching JSON payload values with one bounded SQL query."""
        self._ensure_collection()
        candidates = list(dict.fromkeys(str(value) for value in values if value is not None))
        if not candidates:
            return set()
        if not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", key):
            raise ValueError("Payload key contains unsupported characters")

        filter_conditions, filter_params = _build_filter_conditions(filters)
        conditions = ["payload->>%s = ANY(%s)", *filter_conditions]
        with self._get_cursor() as cur:
            cur.execute(
                sql.SQL("SELECT DISTINCT payload->>%s FROM {} WHERE {}").format(
                    self._col(),
                    sql.SQL(" AND ").join(sql.SQL(condition) for condition in conditions),
                ),
                (key, key, candidates, *filter_params),
            )
            return {str(row[0]) for row in cur.fetchall() if row and row[0] is not None}

    def existing_memory_hashes(self, *, filters: Optional[dict] = None) -> dict[tuple[str, str], str]:
        """Load import identity triples without returning memory text or embeddings."""
        self._ensure_collection()
        filter_conditions, filter_params = _build_filter_conditions(filters)
        conditions = [
        ...
        with self._get_cursor() as cur:
            cur.execute(
                sql.SQL("SELECT id, payload->>'conversation_id', payload->>'hash' FROM {} WHERE {}").format(
                    self._col(),
                    sql.SQL(" AND ").join(sql.SQL(condition) for condition in conditions),
                ),
                tuple(filter_params),
            ...
            return {
                (str(conversation_id), str(memory_hash)): str(memory_id)
                for memory_id, conversation_id, memory_hash in cur.fetchall()
                if conversation_id is not None and memory_hash is not None
            }
    ...
    def atomic_payload_list_union(
        ...
        *,
        ...
        """Atomically union string values into a JSONB payload array.

        PostgreSQL evaluates this expression while holding the row update lock, so concurrent
        workers merge against the latest committed payload instead of overwriting one another's
        links with stale read-modify-write snapshots.
        """
        self._ensure_collection()
        ...
        normalized_values = sorted({value for value in raw_values if value})
            ...
            cur.execute(
                sql.SQL(
                    ...
                    SET payload = jsonb_set(
                        ...
                        (
                            SELECT COALESCE(
                                jsonb_agg(candidate.value ORDER BY candidate.value),
                                '[]'::jsonb
                            )
                            FROM (
                                ...
                                FROM (
                                    SELECT jsonb_array_elements_text(
                                        CASE
                                            WHEN jsonb_typeof(
                                                COALESCE(target.payload, '{{}}'::jsonb) -> %s
                                            ) = 'array'
                                            THEN COALESCE(target.payload, '{{}}'::jsonb) -> %s
                                            ELSE '[]'::jsonb
                                        END
                                    ) AS value
                                    UNION ALL
                                    SELECT unnest(%s::text[]) AS value
                                ) AS combined
                                WHERE value <> ''
                            ) AS candidate
                        ),
                ...
                ([key], key, key, normalized_values, vector_id),
            )

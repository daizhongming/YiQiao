...
def test_entity_store_initializes_once_across_threads(mocker, memory_class):
    ...
    memory._entity_store_lock = threading.Lock()
    ...
    mocker.patch("mem0.memory.main.VectorStoreFactory.create", side_effect=create_store)

    with ThreadPoolExecutor(max_workers=8) as executor:
        stores = list(executor.map(lambda _index: memory.entity_store, range(16)))

    assert create_count == 1
    assert all(store is created_store for store in stores)
...
class TestAddToVectorStoreErrors:
    ...
    def test_invalid_llm_response_fact_extraction_is_reraised(self, mocker, mock_memory, caplog):
        ...
        mock_memory.db.save_messages.assert_not_called()
    ...
    def test_empty_llm_response_is_reraised(self, mock_memory, caplog):
        ...
        # Execute
        with caplog.at_level(logging.WARNING), pytest.raises(LLMError, match="empty response"):
            mock_memory._add_to_vector_store(
                messages=[{"role": "user", "content": "test"}], metadata={}, filters={}, infer=True
            )
...
class TestAsyncAddToVectorStoreErrors:
    ...
    @pytest.mark.asyncio
    async def test_async_invalid_llm_response_fact_extraction_is_reraised(self, mock_async_memory, caplog, mocker):
        ...
        mock_async_memory.db.save_messages.assert_not_called()
        assert any("Error parsing extraction response" in record.message for record in caplog.records), (
            "Expected error message not found in logs"
        )

    @pytest.mark.asyncio
    async def test_async_empty_llm_response_is_reraised(self, mock_async_memory, caplog, mocker):
...
class TestMetadataNotMutated:
    ...
    def test_standard_capacity_callback_counts_raw_records_before_writes(self, mocker):
        memory = _build_memory_instance(mocker, Memory)
        capacity_check = MagicMock(side_effect=RuntimeError("capacity exceeded"))

        with pytest.raises(RuntimeError, match="capacity exceeded"):
            memory._add_to_vector_store(
                [
                    {"role": "system", "content": "not stored"},
                ...
                filters={},
                infer=False,
                pre_vector_write=capacity_check,
            )

        capacity_check.assert_called_once_with(2)
    ...
    def test_memory_add_forwards_standard_capacity_callback_without_import_context(self, mocker):
        memory = _build_memory_instance(mocker, Memory)
        memory.config.llm.config = {"enable_vision": False}
        memory._add_to_vector_store = MagicMock(return_value=[])
        capacity_check = MagicMock()
        ...
        assert memory._add_to_vector_store.call_args.kwargs["pre_vector_write"] is capacity_check
        assert "operation_context" not in memory._add_to_vector_store.call_args.kwargs

    def test_standard_capacity_callback_counts_inferred_post_dedup_records(self, mocker):
        memory = _build_memory_instance(mocker, Memory)
        ...
        memory.llm.generate_response.return_value = (
            '{"memory":['
            '{"text":"first inferred memory"},'
            '{"text":"second inferred memory"},'
            '{"text":"first inferred memory"}'
            "]}"
        )
        capacity_check = MagicMock(side_effect=RuntimeError("capacity exceeded"))
        ...
        with pytest.raises(RuntimeError, match="capacity exceeded"):
            memory._add_to_vector_store(
                [{"role": "user", "content": "infer several facts"}],
                {"project_id": "project-1"},
    ...
    def test_standard_capacity_callback_guards_procedural_memory(self, mocker):
        memory = _build_memory_instance(mocker, Memory)
        memory.config.llm.config = {"enable_vision": False}
        memory.llm.generate_response.return_value = "A durable procedure"
        capacity_check = MagicMock(side_effect=RuntimeError("capacity exceeded"))

        with pytest.raises(RuntimeError, match="capacity exceeded"):
            memory.add(
                [{"role": "user", "content": "Summarize this workflow"}],
                agent_id="agent-1",
                memory_type="procedural_memory",
                pre_vector_write=capacity_check,
            )

        capacity_check.assert_called_once_with(1)
        memory.embedding_model.embed.assert_not_called()
        memory.vector_store.insert.assert_not_called()
        memory.db.add_history.assert_not_called()

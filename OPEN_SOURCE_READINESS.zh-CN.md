...
4. 在受保护的 `main` 上强制执行必需检查，并禁止强制推送和分支删除；随后在该准确提交上
   创建 `v0.1.1`，并确认标签解析到已审查的 `main` SHA。
5. 在 `main` 上调度 `YiQiao Images`，设置 `publish=true` 和 `version=v0.1.1`。如果版本标签
   不存在或指向其他提交，工作流必须拒绝发布。
6. 确认两个 GHCR 软件包均为公开状态，且匿名访问能够解析 `latest`、`v0.1.1` 和完整提交
   SHA 标签；记录每个镜像不可变的多平台摘要。
7. 为 `v0.1.1` 创建中英双语 GitHub Release，其中包含准确的源码 SHA、镜像标签与摘要、

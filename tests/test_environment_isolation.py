...
def test_compose_does_not_expose_retired_oauth_configuration():
    ...
    assert "OAUTH_" not in base
    assert "OAUTH_" not in production
    assert "OAUTH_" not in e2e


def test_smoke_requires_current_migration_head():
    assert full_stack_smoke.EXPECTED_MIGRATION == "019"
...
def test_smoke_rejects_a_stale_migration_head():
    ...
    with pytest.raises(full_stack_smoke.SmokeError, match="Expected Alembic migration 019, found 017"):
...
def test_ci_does_not_run_retired_oauth_concurrency_suite():
    workflow = (PROJECT_ROOT / ".github" / "workflows" / "ci.yml").read_text(encoding="utf-8")

    assert "yiqiao_oauth_test" not in workflow
    assert "OAUTH_TEST_DATABASE_DSN" not in workflow
    assert "test_public_service_oauth_postgres.py" not in workflow

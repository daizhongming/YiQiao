...
class NeptuneAnalyticsVector(VectorStoreBase):
    """
    Neptune Analytics vector store implementation for YiQiao.
    ...
    def list_cols(self):
        """
        List all collections with the legacy compatibility prefix.

        Queries the Neptune Analytics schema to find all node labels that start
        with the configured collection prefix.

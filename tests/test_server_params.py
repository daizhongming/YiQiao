...
@pytest.fixture
def client(_mock_memory, monkeypatch):
    """Return a TestClient wired to the server app with mocked Memory."""
    global _PARAMS_SERVER_READY

    import auth as server_auth
    import server_state

    # Other server test modules may have imported auth before this fixture's
    # environment was installed, so refresh its module-level constants first.
    importlib.reload(server_auth)

    if not _PARAMS_SERVER_READY and "server.main" not in sys.modules:
        import server.main as server_main
    else:
        import server.main as server_main

        if not _PARAMS_SERVER_READY:
            importlib.reload(server_main)
    _PARAMS_SERVER_READY = True
    server_state._memory_instance = _mock_memory
    monkeypatch.setattr(
        server_main,
        "_workspace_settings",
    ...
    for name in (
        ...
        monkeypatch.setattr(server_main, name, MagicMock())
    monkeypatch.setattr(server_main, "graph_related_memories", MagicMock(return_value=[]))
    monkeypatch.setattr(server_main.memories_router, "delete_memory_feedback", MagicMock())
...
def test_request_log_suppression_flag(client):
    import server.main as server_main

    request = SimpleNamespace(
        method="DELETE",
        state=SimpleNamespace(suppress_request_log=True),
        url=SimpleNamespace(path="/api/v1/orgs/organizations/org-test/"),
    )

    assert server_main._should_log_request(request) is False
...
class TestSearchLimit:
    ...
    def test_limit_omitted_uses_memory_default(self, client, mock_memory):
        """Production search defaults are independent from playground settings."""
...
class TestSearchThreshold:
    ...
    def test_production_search_ignores_project_playground_tuning(self, client, mock_memory, monkeypatch):
        import server.main as server_main

        workspace = deepcopy(server_main.DEFAULT_WORKSPACE_SETTINGS)
        workspace["projects"][0]["playground"] = {
            "top_k": 2,
            "threshold": 0.91,
            "reranking": True,
        }
        monkeypatch.setattr(server_main, "_workspace_settings", MagicMock(return_value=workspace))
...
class TestAddInfer:
    ...
    def test_infer_omitted_uses_environment_default(self, client, mock_memory):
        resp = client.post(
            "/memories",
            json={
    ...
    def test_auth_disabled_add_receives_noop_capacity_callback(self, client, mock_memory):
        resp = client.post(
            "/memories",
            json={
                "messages": [{"role": "user", "content": "Store without a policy"}],
                "user_id": "u1",
                "infer": False,
            },
        )
        ...
        assert resp.status_code == 200
        _, kwargs = mock_memory.add.call_args
        capacity_check = kwargs["pre_vector_write"]
        capacity_check(100)

    def test_infer_omitted_can_use_false_environment_default(self, client, mock_memory):
        with patch("server.main.DEFAULT_MEMORY_INFER", False):
            resp = client.post(
                "/memories",
                json={
                    "messages": [{"role": "user", "content": "hello"}],
    ...
    def test_disabled_retention_does_not_inject_default_expiration(self, client, mock_memory, monkeypatch):
        import server.main as server_main

        workspace = deepcopy(server_main.DEFAULT_WORKSPACE_SETTINGS)
        workspace["projects"][0]["retention"] = {
            "memory_decay": False,
    ...
    def test_categories_are_normalized_into_metadata(self, client, mock_memory):
        resp = client.post(
            "/memories",
            json={
                ...
                "metadata": {"category": "legacy", "source": "test"},
            },
        )
        assert resp.status_code == 200
        _, kwargs = mock_memory.add.call_args
        assert "categories" not in kwargs
        assert kwargs["metadata"]["categories"] == ["user_preference"]
        assert kwargs["metadata"]["source"] == "test"
        assert "category" not in kwargs["metadata"]


# ===========================================================================
# Generate categories from extraction settings
# ===========================================================================


class TestGenerateCategories:
    """Verify generated category definitions are normalized before returning."""

    def test_generate_categories_normalizes_llm_output(self, client, mock_memory):
        mock_memory.llm.generate_response.return_value = """
        {
          "categories": [
            {"name": "User Preference", "description": "Preferences and style choices"},
            {"name": "user-preference", "description": "duplicate"},
            {"name": "Project Detail", "description": "Project facts"}
          ]
        }
        """
        ...
        resp = client.post(
            ...
            json={
                ...
                "include": "preferences, projects",
                "exclude": "temporary chat state",
                "multilingual": True,
                "custom_instructions": "Keep categories durable.",
            },
        )

        assert resp.status_code == 200
        assert resp.json()["categories"] == [
            {"name": "user_preference", "description": "Preferences and style choices"},
            {"name": "project_detail", "description": "Project facts"},
        ]
        args, kwargs = mock_memory.llm.generate_response.call_args
        assert "Coding assistant" in args[0][0]["content"]
        assert kwargs["response_format"] == {"type": "json_object"}
        assert kwargs["max_tokens"] == 8192
        assert kwargs["temperature"] == 0
        assert kwargs["timeout"] == 60.0

    def test_generate_categories_rejects_incomplete_json(self, client, mock_memory):
        mock_memory.llm.generate_response.return_value = '{"categories":[{"name":"career"'

        resp = client.post(
            "/generate-categories",
            json={"use_case": "Job search"},
        )

        assert resp.status_code == 502
        assert resp.json()["detail"] == "The LLM returned incomplete category JSON. Please retry."


class TestGenerateInstructions:
    def test_generate_instructions_parses_complete_response(self, client, mock_memory):
        mock_memory.llm.generate_response.return_value = (
            "INSTRUCTIONS: Store durable career facts.\nTEST_MESSAGE: I am applying for a backend engineering role."
        )

        resp = client.post(
            "/generate-instructions",
            json={"use_case": "Job search", "memory_depth": "Comprehensive Knowledge"},
        )

        assert resp.status_code == 200
        assert resp.json() == {
            "custom_instructions": "Store durable career facts.",
            "test_message": "I am applying for a backend engineering role.",
        }
        _args, kwargs = mock_memory.llm.generate_response.call_args
        assert kwargs == {"max_tokens": 8192, "temperature": 0, "timeout": 60.0}

    def test_generate_instructions_rejects_truncated_response(self, client, mock_memory):
        mock_memory.llm.generate_response.return_value = "INSTRUCTIONS: Store durable career facts"

        resp = client.post("/generate-instructions", json={"use_case": "Job search"})

        assert resp.status_code == 502
        assert resp.json()["detail"] == "The LLM returned an incomplete instruction response. Please retry."
...
class TestAddPrompt:
    ...
    def test_prompt_omitted(self, client, mock_memory):
        ...
        assert resp.status_code == 200
        _, kwargs = mock_memory.add.call_args
        assert "Extract only stable facts" in kwargs["prompt"]
...
class TestExistingParamsUnchanged:
    ...
    def test_add_metadata_still_forwarded(self, client, mock_memory):
        ...
        assert kwargs["metadata"] == {"source": "test", "project_id": "default-project"}
...
class TestExplicitNull:
    ...
    def test_infer_null_uses_environment_default(self, client, mock_memory):
        resp = client.post(
            "/memories",
            json={
                "messages": [{"role": "user", "content": "test"}],
...
class TestUpdateMemory:
    ...
    def test_metadata_forwarded(self, client, mock_memory):
        ...
        assert resp.status_code == 200
        _, kwargs = mock_memory.update.call_args
        assert kwargs["metadata"] == {"categories": ["sports"], "project_id": "default-project"}
...
class TestGetMemories:
    ...
    def test_get_memories_entity_filters_routing(self, client, mock_memory):
        ...
        assert kwargs["filters"] == {"user_id": "test_routing_user", "project_id": "default-project"}
    ...
    def test_project_member_can_list_current_project_memories(self, client, mock_memory):
        from types import SimpleNamespace
        ...
        request = Request({"type": "http", "method": "GET", "path": "/memories", "headers": []})
        request.state.auth_type = "bearer"
        request.state.project_id = "project_a"
        mock_memory.vector_store.list.return_value = [
            SimpleNamespace(id="mem-a", payload={"data": "Project A memory", "project_id": "project_a"}),
            SimpleNamespace(id="mem-b", payload={"data": "Project B memory", "project_id": "project_b"}),
        ]

        response = server_main.get_all_memories(
            request,
            top_k=1000,
            _auth=SimpleNamespace(role="user"),
        )

        assert response["results"] == [
            {
                "id": "mem-a",
                "memory": "Project A memory",
...
class TestSearchEntityIdMapping:
    ...
    def test_no_entity_ids_no_filters(self, client, mock_memory):
        ...
        assert resp.status_code == 200
        _, kwargs = mock_memory.search.call_args
        assert kwargs["filters"] == {"project_id": "default-project"}
...
class TestWriteHandlerErrorMapping:
    ...
    def test_add_capacity_error_returns_structured_429_and_releases_guard(
        self,
        client,
        mock_memory,
        monkeypatch,
        ...
        guard = MagicMock()
        guard.side_effect = server_main.ImportStorageQuotaExceeded(
            scope_type="project",
        ...
        monkeypatch.setattr(server_main, "ImportStorageQuotaGuard", MagicMock(return_value=guard))
        mock_memory.add.side_effect = lambda **kwargs: kwargs["pre_vector_write"](2)

        resp = client.post(
            "/memories",
            json={"messages": [{"role": "user", "content": "hi"}], "user_id": "u1"},
        )

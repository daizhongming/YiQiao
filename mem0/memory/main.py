...
from mem0.configs.prompts import (
    ...
    IMPORT_EXTRACTION_SUFFIX,
    PROCEDURAL_MEMORY_SYSTEM_PROMPT,
    generate_additive_extraction_prompt,
)
from mem0.exceptions import EmbeddingError, LLMError
...
class MemoryOperationContext:
    """Optional controls and results for a single import-oriented ``Memory.add`` call.

    The default ``Memory.add`` path is unchanged when this context is omitted. Instances are
    intentionally mutable: the pipeline writes model selection, audit metadata, timings, and
    claimed hashes back for the import coordinator to persist or release after a failure.
    """

    primary_llm: Any = None
    fallback_llm: Any = None
    primary_model_label: str = "primary"
    fallback_model_label: str = "fallback"
    force_fallback: bool = False
    force_fallback_reason: Optional[str] = None
    require_source_message_indices: bool = False
    source_messages: Optional[list[Mapping[str, Any]]] = None
    core_source_message_indices: Optional[list[int]] = None
    min_confidence: float = 0.0
    audit: bool = False
    audit_use_more_complete: bool = False
    obvious_fact_empty_fallback: bool = False
    strict_vector_write: bool = False
    phase_callback: Optional[Callable[[str, float], None]] = None
    memory_hash_claim: Optional[Callable[[str], bool]] = None
    ...
    model_used: Optional[str] = field(default=None, init=False)
    fallback_reason: Optional[str] = field(default=None, init=False)
    audit_result: Optional[str] = field(default=None, init=False)
    audit_metadata: Dict[str, Any] = field(default_factory=dict, init=False)
    provider_pressure_recovered: bool = field(default=False, init=False)
    claimed_memory_hashes: list[str] = field(default_factory=list, init=False)
    phase_timings: Dict[str, float] = field(default_factory=dict, init=False)

    def __post_init__(self) -> None:
        boolean_fields = (
            "force_fallback",
            "require_source_message_indices",
            "audit",
            "audit_use_more_complete",
            "obvious_fact_empty_fallback",
            "strict_vector_write",
        )
        for field_name in boolean_fields:
            if not isinstance(getattr(self, field_name), bool):
                raise TypeError(f"{field_name} must be bool")
        if self.core_source_message_indices is not None:
            if not isinstance(self.core_source_message_indices, (list, tuple)):
                raise TypeError("core_source_message_indices must be a list of non-negative integers or None")
            if not self.core_source_message_indices:
                raise ValueError("core_source_message_indices must not be empty")
            if any(
                isinstance(index, bool) or not isinstance(index, int) or index < 0
                for index in self.core_source_message_indices
            ):
                raise ValueError("core_source_message_indices must contain only non-negative integers")
            if len(self.core_source_message_indices) != len(set(self.core_source_message_indices)):
                raise ValueError("core_source_message_indices must not contain duplicates")
            self.core_source_message_indices = list(self.core_source_message_indices)
        for field_name in ("execution_guard", "pre_vector_write"):
    ...
    def reset_results(self) -> None:
        """Clear per-call outputs before reusing a context instance."""
        self.model_used = None
        self.fallback_reason = None
        self.audit_result = None
        self.audit_metadata.clear()
        self.provider_pressure_recovered = False
        self.claimed_memory_hashes.clear()
        self.phase_timings.clear()


def _run_import_execution_guard(operation_context: Optional[MemoryOperationContext]) -> None:
    if operation_context is not None and operation_context.execution_guard is not None:
        operation_context.execution_guard()


class _ImportExtractionError(LLMError):
    def __init__(self, reason: str, message: str):
        self.reason = reason
        super().__init__(message)


def _is_provider_pressure_exception(exc: BaseException) -> bool:
    ...
    while current is not None and id(current) not in seen:
        ...
        for candidate in (
            ...
            try:
                if candidate is not None and int(candidate) in {408, 429, 502, 503, 504}:
                    return True
    ...
    return any(marker in error_text for marker in ("429", "rate limit", "too many requests", "timed out", "timeout"))


class _OperationPhaseTimer:
    _PHASE_ORDER = ("context_load", "llm", "embedding", "pgvector", "entity_processing")

    def __init__(self, operation_context: MemoryOperationContext):
    ...
    def flush(self) -> None:
        if self._flushed:
            return
        self._flushed = True
        callback = self.operation_context.phase_callback
        for phase in self._PHASE_ORDER:
            if phase not in self.durations:
                continue
            seconds = float(self.durations[phase])
            self.operation_context.phase_timings[phase] = seconds
            if callback is None:
                continue
            try:
                callback(phase, seconds)
            except Exception:
                logger.warning("Memory operation phase callback failed for phase '%s'", phase)


_OBVIOUS_FACT_PATTERNS = (
    re.compile(
        r"\b(?:i am|i'm|i live|i work|i prefer|i like|i love|i hate|i have|i own|"
        r"i plan|i intend|i will|my (?:name|favorite|favourite|job|role|home|birthday) is)\b",
        re.IGNORECASE,
    ),
    re.compile(
        r"(?:\u6211\u53eb|\u6211\u662f|\u6211\u4f4f\u5728|\u6211\u5728.+\u5de5\u4f5c|"
    ...
    re.compile(r"\b(?:i recommend|you should|the plan (?:is|includes))\b", re.IGNORECASE),
    re.compile(r"(?:\u6211\u5efa\u8bae|\u5efa\u8bae\u4f60|\u8ba1\u5212\u5305\u62ec)"),
)


def _parse_extraction_response(response: Any) -> list[dict[str, Any]]:
    """Parse the extraction contract without turning malformed output into a no-op."""
    if isinstance(response, dict):
        payload = response
    else:
        if not isinstance(response, str) or not response.strip():
            raise LLMError("LLM extraction returned an empty response.")
        ...
        try:
            ...
            try:
                ...
                raise LLMError(f"LLM extraction returned invalid JSON ({len(cleaned)} characters).") from exc

    if not isinstance(payload, dict):
        raise LLMError("LLM extraction response must be a JSON object.")
    if "memory" not in payload:
        raise LLMError("LLM extraction response is missing the 'memory' field.")

    memories = payload["memory"]
    if not isinstance(memories, list):
        raise LLMError("LLM extraction response field 'memory' must be a list.")
    for index, memory in enumerate(memories):
        if not isinstance(memory, dict) or not isinstance(memory.get("text"), str):
            raise LLMError(f"LLM extraction memory at index {index} must contain string field 'text'.")
    return memories


def _normalize_operation_source_messages(
    operation_context: MemoryOperationContext,
    messages: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    supplied_messages = operation_context.source_messages
    raw_messages = supplied_messages if supplied_messages is not None else messages
    if not isinstance(raw_messages, (list, tuple)) or not raw_messages:
        raise ValueError("source_messages must contain at least one message")
    ...
    for position, raw_message in enumerate(raw_messages):
        ...
        if not isinstance(raw_message, Mapping):
            raise ValueError(f"source_messages[{position}] must be a mapping")

        source_index = raw_message.get("source_index")
        if source_index is None and supplied_messages is None:
            source_index = position
        if isinstance(source_index, bool) or not isinstance(source_index, int) or source_index < 0:
            raise ValueError(f"source_messages[{position}].source_index must be a non-negative integer")
        if source_index in seen_indices:
            raise ValueError(f"source_messages contains duplicate source_index {source_index}")

        role = raw_message.get("role")
        content = raw_message.get("content")
        if not isinstance(role, str) or not role.strip():
            raise ValueError(f"source_messages[{position}].role must be a non-empty string")
        if not isinstance(content, str) or not content.strip():
            raise ValueError(f"source_messages[{position}].content must be a non-empty string")
...
def _operation_core_source_indices(
    operation_context: MemoryOperationContext,
    source_messages: list[dict[str, Any]],
) -> tuple[list[int], bool]:
    """Return core indices in source order and whether this chunk has overlap context."""
    available_indices = [message["source_index"] for message in source_messages]
    configured_indices = operation_context.core_source_message_indices
    if configured_indices is None:
        return available_indices, False

    available = set(available_indices)
    unavailable = [index for index in configured_indices if index not in available]
    if unavailable:
        raise ValueError("core_source_message_indices contains an unavailable source index")
    configured = set(configured_indices)
    core_indices = [index for index in available_indices if index in configured]
    return core_indices, len(core_indices) < len(available_indices)


def _import_core_evidence_prompt(
    ...
    if not has_overlap:
        return ""
    serialized_indices = json.dumps(core_indices, ensure_ascii=True, separators=(",", ":"))
    core_index_set = set(core_indices)
    context_only_indices = [index for index in available_indices if index not in core_index_set]
    serialized_context_indices = json.dumps(context_only_indices, ensure_ascii=True, separators=(",", ":"))
    return (
        "\n\nThis chunk includes leading overlap messages for context. The core source_message_indices are "
        f"{serialized_indices}. Every emitted memory MUST cite at least one of these core indices in "
        "source_message_indices. The context-only source_message_indices are "
        f"{serialized_context_indices}; NEVER cite those indices as the sole evidence for a memory. "
        "Do not emit a memory supported only by overlap messages; those messages were processed in an "
        "earlier chunk. If the core messages contain no new memory, return an empty memory list."
    )


def _has_obvious_fact(source_messages: list[dict[str, Any]]) -> bool:
    for message in source_messages:
        if message.get("role") == "system":
            continue
        content = message.get("content", "")
        if any(pattern.search(content) for pattern in _OBVIOUS_FACT_PATTERNS):
            return True
    return False


def _is_conflicted_memory(memory: Mapping[str, Any]) -> bool:
    for key in ("conflict", "has_conflict", "conflicting", "conflict_detected"):
        value = memory.get(key)
        if value is True:
            return True
        if isinstance(value, (str, list, tuple, dict, set)) and bool(value):
            return True
    conflicts = memory.get("conflicts")
    return isinstance(conflicts, (str, list, tuple, dict, set)) and bool(conflicts)


def _validate_import_extraction(
    response: Any,
    source_messages: list[dict[str, Any]],
    operation_context: MemoryOperationContext,
) -> list[dict[str, Any]]:
    try:
        ...
        raise _ImportExtractionError(reason, f"Import extraction failed validation: {reason}") from exc

    source_by_index = {message["source_index"]: message for message in source_messages}
    core_indices, has_overlap = _operation_core_source_indices(operation_context, source_messages)
    core_index_set = set(core_indices)
    normalized_memories = []
    overlap_only_positions = []
    for position, memory in enumerate(memories):
        text = memory.get("text")
        if not isinstance(text, str) or not text.strip():
            raise _ImportExtractionError(
                "invalid_schema",
                f"Import extraction memory at index {position} has an invalid text field.",
            )
        if "id" in memory and not isinstance(memory["id"], str):
            raise _ImportExtractionError(
                "invalid_schema",
                f"Import extraction memory at index {position} has an invalid id field.",
            )
        if "attributed_to" in memory and memory["attributed_to"] not in {"user", "assistant"}:
            raise _ImportExtractionError(
                "invalid_schema",
                f"Import extraction memory at index {position} has an invalid attributed_to field.",
            )
        if "linked_memory_ids" in memory and (
            not isinstance(memory["linked_memory_ids"], list)
            or not all(isinstance(value, str) for value in memory["linked_memory_ids"])
        ):
            raise _ImportExtractionError(
                "invalid_schema",
                f"Import extraction memory at index {position} has invalid linked_memory_ids.",
            )
        if _is_conflicted_memory(memory):
            raise _ImportExtractionError(
                "conflict",
                f"Import extraction memory at index {position} contains unresolved conflict.",
            )
        ...
        if isinstance(confidence, bool) or not isinstance(confidence, (int, float)):
            raise _ImportExtractionError(
                "invalid_schema",
                f"Import extraction memory at index {position} is missing numeric confidence.",
            )
        confidence = float(confidence)
        if not math.isfinite(confidence) or not 0.0 <= confidence <= 1.0:
            raise _ImportExtractionError(
                "invalid_schema",
                f"Import extraction memory at index {position} has confidence outside 0.0 through 1.0.",
            )
        if confidence < operation_context.min_confidence:
            raise _ImportExtractionError(
                "low_confidence",
                f"Import extraction memory at index {position} is below the confidence threshold.",
            )

        source_indices = memory.get("source_message_indices")
        if source_indices is None and not operation_context.require_source_message_indices:
            source_indices = []
        if not isinstance(source_indices, list) or (
            operation_context.require_source_message_indices and not source_indices
        ):
            raise _ImportExtractionError(
                "missing_evidence",
                f"Import extraction memory at index {position} is missing source evidence.",
            )
        if any(isinstance(index, bool) or not isinstance(index, int) for index in source_indices):
            raise _ImportExtractionError(
                "invalid_schema",
                f"Import extraction memory at index {position} has non-integer source indices.",
            )
        if len(source_indices) != len(set(source_indices)):
            raise _ImportExtractionError(
                "invalid_schema",
                f"Import extraction memory at index {position} has duplicate source indices.",
            )
        missing_indices = [index for index in source_indices if index not in source_by_index]
        if missing_indices:
            raise _ImportExtractionError(
                "source_index_out_of_range",
                f"Import extraction memory at index {position} cites an unavailable source index.",
            )
        evidence = [source_by_index[index] for index in source_indices]
        if source_indices and not any(item["role"] != "system" and item["content"].strip() for item in evidence):
            raise _ImportExtractionError(
                "missing_evidence",
                f"Import extraction memory at index {position} cites no usable evidence.",
            )
        if has_overlap and not core_index_set.intersection(source_indices):
            overlap_only_positions.append(position)
            continue

        normalized_memory = dict(memory)
        normalized_memory["text"] = text.strip()
        normalized_memory["confidence"] = confidence
        normalized_memory["source_message_indices"] = list(source_indices)
        normalized_memories.append(normalized_memory)
    if overlap_only_positions and not normalized_memories:
        position = overlap_only_positions[0]
        raise _ImportExtractionError(
            "missing_core_evidence",
            f"Import extraction memory at index {position} cites only overlap evidence.",
        )
    return normalized_memories


def _extraction_quality(memories: list[dict[str, Any]]) -> tuple[int, int, float]:
    covered_sources = {
        source_index
        for memory in memories
        for source_index in memory.get("source_message_indices", [])
        if isinstance(source_index, int) and not isinstance(source_index, bool)
    }
    confidence_total = sum(float(memory.get("confidence", 0.0)) for memory in memories)
    return len(covered_sources), len(memories), confidence_total


def _audit_summary(memories: list[dict[str, Any]]) -> Dict[str, Any]:
    coverage, count, confidence_total = _extraction_quality(memories)
    return {
        "memory_count": count,
        "source_coverage": coverage,
        "average_confidence": confidence_total / count if count else 0.0,
    }


def _category_match_key(value):
    return " ".join(str(value or "").replace("_", " ").replace("-", " ").lower().split())


def _normalize_memory_categories(value, allowed=None):
    if value is None or isinstance(value, bool):
        return []
    raw_items = value if isinstance(value, (list, tuple, set)) else [value]
    allowed_map = None
    if allowed:
        allowed_map = {_category_match_key(item): str(item).strip() for item in allowed if str(item or "").strip()}
    categories = []
    seen = set()
...
def _format_added_memory(record):
    memory_id, text, _embedding, payload = record
    item = {"id": memory_id, "memory": text, "event": "ADD"}
    categories = _normalize_memory_categories((payload or {}).get("categories") or (payload or {}).get("category"))
    if categories:
        item["categories"] = categories
    source_indices = (payload or {}).get("source_message_indices")
    if isinstance(source_indices, list):
        item["source_message_indices"] = list(source_indices)
...
_SESSION_SCOPE_KEYS = ("agent_id", "app_id", "project_id", "run_id", "user_id")
...
def _build_filters_and_metadata(
    *,  # Enforce keyword-only arguments
    ...
    This helper supports multiple session identifiers (`user_id`, `agent_id`, `app_id`, and/or `run_id`)
    ...
    app_id = _validate_and_trim_entity_id(app_id or base_metadata_template.get("app_id"), "app_id")
    run_id = _validate_and_trim_entity_id(run_id, "run_id")
    project_id = _validate_and_trim_entity_id(base_metadata_template.get("project_id"), "project_id")
...
def _build_session_scope(filters):
    """Build a deterministic conversation scope from entity and storage IDs."""
    parts = []
    for key in _SESSION_SCOPE_KEYS:
        val = filters.get(key)
        if val:
...
class Memory(MemoryBase):
    ...
    def _existing_entities_by_text(self, filters, *, strict: bool = False):
    ...
    def _union_entity_linked_memory_ids(
        self,
        match,
        memory_ids,
        *,
        ...
        """Add entity links atomically when the vector store provides that capability."""
        new_ids = sorted({memory_id for memory_id in memory_ids if isinstance(memory_id, str) and memory_id})
        if not new_ids:
            return True

        entity_store = self.entity_store
        atomic_union = getattr(type(entity_store), "atomic_payload_list_union", None)
        if callable(atomic_union):
            _run_import_execution_guard(operation_context)
            updated = atomic_union(
                entity_store,
                vector_id=match.id,
            ...
            if not updated and strict:
                raise RuntimeError(f"Entity row disappeared before linked-memory update: {match.id}")
            return bool(updated)

        payload = dict(match.payload or {})
        existing_ids = {
            memory_id for memory_id in payload.get("linked_memory_ids", []) if isinstance(memory_id, str) and memory_id
        }
        linked_ids = existing_ids | set(new_ids)
        if linked_ids == existing_ids:
            return True
        payload["linked_memory_ids"] = sorted(linked_ids)
        _run_import_execution_guard(operation_context)
        entity_store.update(vector_id=match.id, vector=None, payload=payload)
        return True
    ...
    def _upsert_entity(self, entity_text, entity_type, memory_id, filters):
        ...
        try:
            ...
            if match:
                self._union_entity_linked_memory_ids(match, [memory_id])
            else:
                # Create new entity
    ...
    def add(
        ...
        *,
        ...
        Adds new memories scoped to a session id (for example, `user_id`, `agent_id`, `app_id`,
        or `run_id`). One of those ids is required.
        ...
        Args:
            ...
            operation_context (MemoryOperationContext, optional): Import-specific extraction,
                fallback, audit, traceability, strict-write, and timing controls. Omitting it
                preserves the standard add pipeline.
            pre_vector_write (callable, optional): Standard-pipeline capacity check invoked once
                with the exact number of prospective new records before persistence.
        ...
        if operation_context is not None and not isinstance(operation_context, MemoryOperationContext):
            raise TypeError("operation_context must be a MemoryOperationContext instance or None")
        if pre_vector_write is not None and not callable(pre_vector_write):
            raise TypeError("pre_vector_write must be callable or None")
        if operation_context is not None and pre_vector_write is not None:
            raise ValueError("pre_vector_write cannot be combined with MemoryOperationContext")
    ...
    def _add_to_vector_store(
        ...
        if operation_context is not None:
            ...
            if pre_vector_write is not None:
                raise ValueError("pre_vector_write cannot be combined with MemoryOperationContext")
            return self._add_to_vector_store_with_operation_context(
                messages,
                metadata,
                filters,
                prompt,
        ...
        if not infer:
            ...
            for message_dict in valid_messages:
                ...
                categories = _normalize_memory_categories(
                    per_msg_meta.get("categories") or per_msg_meta.get("category"),
                    allowed_categories,
                )
                if categories:
                    per_msg_meta["categories"] = categories
                else:
                    per_msg_meta.pop("categories", None)
        ...
        extracted_memories = _parse_extraction_response(response)
            ...
            if global_entities:
                ...
                if valid:
                    ...
                    for j, key in enumerate(valid_keys):
                        ...
                        if match:
                            try:
                                self._union_entity_linked_memory_ids(match, memory_ids)
                            except Exception as e:
        ...
        returned_memories = [_format_added_memory(r) for r in records]
    ...
    def _run_import_extraction_model(
        self,
        model,
        llm_messages,
        source_messages,
        ...
        if model is None or not callable(getattr(model, "generate_response", None)):
            raise _ImportExtractionError("model_unavailable", "Import extraction model is unavailable.")
        try:
            ...
            raise _ImportExtractionError("model_error", "Import extraction model call failed.") from exc
        return _validate_import_extraction(response, source_messages, operation_context)

    def _extract_import_memories(
        self,
        llm_messages,
        source_messages,
        operation_context: MemoryOperationContext,
    ):
        primary_model = operation_context.primary_llm or self.llm
        fallback_model = operation_context.fallback_llm
        primary_label = operation_context.primary_model_label or "primary"
        fallback_label = operation_context.fallback_model_label or "fallback"
        obvious_fact_present = operation_context.obvious_fact_empty_fallback and _has_obvious_fact(source_messages)

        def run_model(model):
            memories = self._run_import_extraction_model(
                model,
                llm_messages,
                source_messages,
                operation_context,
            )
            if obvious_fact_present and not memories:
                raise _ImportExtractionError(
                    "empty_obvious_fact",
                    "Import extraction returned no memories despite an obvious fact.",
                )
            return memories

        if operation_context.force_fallback:
            if fallback_model is None:
                raise LLMError("force_fallback requires fallback_llm in MemoryOperationContext.")
            operation_context.model_used = fallback_label
            operation_context.fallback_reason = operation_context.force_fallback_reason or "forced_fallback"
            memories = run_model(fallback_model)
            if operation_context.audit:
                operation_context.audit_result = "forced_fallback"
                operation_context.audit_metadata = {
                    "primary_valid": False,
                    "fallback_valid": True,
                    "fallback": _audit_summary(memories),
                    "selected_model": fallback_label,
                }
            return memories

        try:
            primary_memories = run_model(primary_model)
        except _ImportExtractionError as primary_error:
            operation_context.model_used = primary_label
            operation_context.fallback_reason = f"primary_{primary_error.reason}"
            operation_context.provider_pressure_recovered = _is_provider_pressure_exception(primary_error)
            if fallback_model is None:
                raise
            operation_context.model_used = fallback_label
            try:
                fallback_memories = run_model(fallback_model)
            except _ImportExtractionError as fallback_error:
                if operation_context.audit:
                    operation_context.audit_result = "fallback_invalid"
                    operation_context.audit_metadata = {
                        "primary_valid": False,
                        "fallback_valid": False,
                        "primary_failure": primary_error.reason,
                        "fallback_failure": fallback_error.reason,
                        "selected_model": None,
                    }
                raise LLMError("Fallback import extraction failed validation.") from fallback_error
            if operation_context.audit:
                operation_context.audit_result = "primary_invalid_fallback_selected"
                operation_context.audit_metadata = {
                    "primary_valid": False,
                    "fallback_valid": True,
                    "primary_failure": primary_error.reason,
                    "fallback": _audit_summary(fallback_memories),
                    "selected_model": fallback_label,
                }
            return fallback_memories

        operation_context.model_used = primary_label
        if not operation_context.audit:
            return primary_memories
        if fallback_model is None:
            raise LLMError("audit requires fallback_llm in MemoryOperationContext.")

        try:
            fallback_memories = run_model(fallback_model)
        except _ImportExtractionError as fallback_error:
            operation_context.provider_pressure_recovered = _is_provider_pressure_exception(fallback_error)
            operation_context.audit_result = "fallback_invalid"
            operation_context.audit_metadata = {
                "primary_valid": True,
                "fallback_valid": False,
                "primary": _audit_summary(primary_memories),
                "fallback_failure": fallback_error.reason,
                "selected_model": primary_label,
            }
            return primary_memories

        fallback_more_complete = _extraction_quality(fallback_memories) > _extraction_quality(primary_memories)
        select_fallback = operation_context.audit_use_more_complete and fallback_more_complete
        operation_context.audit_metadata = {
            "primary_valid": True,
            "fallback_valid": True,
            "primary": _audit_summary(primary_memories),
            "fallback": _audit_summary(fallback_memories),
            "fallback_more_complete": fallback_more_complete,
            "selected_model": fallback_label if select_fallback else primary_label,
        }
        if select_fallback:
            operation_context.model_used = fallback_label
            operation_context.fallback_reason = "audit_more_complete"
            operation_context.audit_result = "fallback_selected"
            return fallback_memories

        operation_context.audit_result = (
            "fallback_more_complete_not_selected" if fallback_more_complete else "primary_kept"
        )
        return primary_memories

    def _process_import_entities(self, records, search_filters, operation_context=None):
        """Batch-link import entities, raising so committed vectors can drive a retry."""
        lock = getattr(self, "_entity_link_lock", None)
        if lock is None:
            lock = threading.RLock()
            self._entity_link_lock = lock
        with lock:
            return self._process_import_entities_locked(records, search_filters, operation_context)

    def _process_import_entities_locked(self, records, search_filters, operation_context=None):
        all_entities = extract_entities_batch([record[1] for record in records])
        if len(all_entities) != len(records):
            raise RuntimeError("Import entity extraction returned an incomplete batch.")
        global_entities = {}
        for index, record in enumerate(records):
            memory_id = record[0]
            entities = all_entities[index] if index < len(all_entities) else []
            for entity_type, entity_text in entities:
                key = self._normalize_entity_text(entity_text)
                if not key:
                    continue
        ...
        ordered_keys = list(global_entities)
        entity_texts = [global_entities[key][1] for key in ordered_keys]
        batch_embedding_error = None
        try:
            entity_embeddings = list(self.embedding_model.embed_batch(entity_texts, "add"))
        except Exception as exc:
            batch_embedding_error = exc
            entity_embeddings = []
            for entity_text in entity_texts:
                try:
                    entity_embeddings.append(self.embedding_model.embed(entity_text, "add"))
                except Exception as item_error:
                    raise EmbeddingError("Import entity embedding failed.") from item_error
        if len(entity_embeddings) != len(ordered_keys) or any(embedding is None for embedding in entity_embeddings):
            raise EmbeddingError("Import entity embedding failed.") from batch_embedding_error

        valid_vectors = entity_embeddings
        valid_texts = entity_texts
        exact_matches = self._existing_entities_by_text(search_filters, strict=True)
        existing_matches = self.entity_store.search_batch(
            queries=valid_texts,
            vectors_list=valid_vectors,
            top_k=1,
        ...
        if len(existing_matches) != len(ordered_keys):
            raise RuntimeError("Import entity lookup returned an incomplete batch.")

        entity_scope = json.dumps(search_filters, ensure_ascii=True, sort_keys=True, separators=(",", ":"))
        insert_vectors = []
        insert_ids = []
        insert_payloads = []
        for result_index, key in enumerate(ordered_keys):
            entity_type, entity_text, memory_ids = global_entities[key]
            matches = existing_matches[result_index] if result_index < len(existing_matches) else []
            exact_match = exact_matches.get(key)
            semantic_match = matches[0] if matches and matches[0].score >= 0.95 else None
            match = exact_match or semantic_match
            if match:
                self._union_entity_linked_memory_ids(
                    match,
                    memory_ids,
                    strict=True,
                    operation_context=operation_context,
            ...
            insert_vectors.append(valid_vectors[result_index])
            insert_ids.append(str(uuid.uuid5(uuid.NAMESPACE_URL, f"mem0:import-entity:{entity_scope}:{key}")))
        ...
        return [
            {
                "id": str(
                    uuid.uuid5(
                        uuid.NAMESPACE_URL,
                        f"mem0:import-history:add:{record[0]}",
                    )
                ),
                "memory_id": record[0],
                "old_memory": None,
                ...
                "is_deleted": 0,
                "project_id": record[3].get("project_id"),
            }
            for record in records
        ]

    @staticmethod
    def _import_message_ids(source_messages, metadata, session_scope):
        import_key = str((metadata or {}).get("import_key") or "")
        if not import_key:
            return None
        conversation_id = str((metadata or {}).get("conversation_id") or "")
        project_id = str((metadata or {}).get("project_id") or "")
        source_app = str((metadata or {}).get("source_app") or "")
        ...
        for position, message in enumerate(source_messages):
            ...
            if conversation_id and isinstance(source_index, int) and not isinstance(source_index, bool):
                ...
                occurrence_key = (source_index, message_fingerprint)
                occurrence = fingerprint_occurrences.get(occurrence_key, 0)
                fingerprint_occurrences[occurrence_key] = occurrence + 1
                identity = json.dumps(
                    {
                ...
                id_prefix = "mem0:import-message:v2"
            else:
                identity = f"{session_scope}:{import_key}:{position}"
                id_prefix = "mem0:import-message"
            message_ids.append(str(uuid.uuid5(uuid.NAMESPACE_URL, f"{id_prefix}:{identity}")))
        return message_ids

    def _save_import_messages(self, source_messages, metadata, session_scope, operation_context=None):
        _run_import_execution_guard(operation_context)
        self.db.save_messages(
            source_messages,
            session_scope,
            message_ids=self._import_message_ids(source_messages, metadata, session_scope),
        )

    def _complete_import_side_effects(
        self,
        records,
        source_messages,
        filters,
        ...
        *,
        ...
        history_records = self._import_history_records(records)
        _run_import_execution_guard(operation_context)
        self.db.batch_add_history(
            history_records,
            deduplicate_by_memory_event=True,
        )
        search_filters = {key: value for key, value in filters.items() if key in _SESSION_SCOPE_KEYS and value}
        if phase_timer is None:
            self._process_import_entities(records, search_filters, operation_context)
        else:
            with phase_timer.track("entity_processing"):
                self._process_import_entities(records, search_filters, operation_context)
        self._save_import_messages(
            source_messages,
            metadata,
    ...
    def _add_to_vector_store_with_operation_context(
        self,
        messages,
        metadata,
        filters,
        ...
        operation_context.reset_results()
        timer = _OperationPhaseTimer(operation_context)
        try:
            source_messages = _normalize_operation_source_messages(operation_context, messages)
            core_source_indices, has_overlap = _operation_core_source_indices(operation_context, source_messages)
            session_scope = _build_session_scope(filters)
            with timer.track("context_load"):
                last_messages = self.db.get_last_messages(session_scope, limit=10)
            parsed_messages = parse_messages(messages)
            ...
            try:
                ...
                raise EmbeddingError("Import query embedding failed.") from exc

            search_filters = {key: value for key, value in filters.items() if key in _SESSION_SCOPE_KEYS and value}
            with timer.track("pgvector"):
                existing_results = self.vector_store.search(
                    query=parsed_messages,
                    vectors=query_embedding,
                    top_k=10,
            ...
            for index, memory in enumerate(existing_results):
                uuid_mapping[str(index)] = memory.id
                existing_memories.append({"id": str(index), "text": memory.payload.get("data", "")})

            is_agent_scoped = bool(filters.get("agent_id")) and not filters.get("user_id")
            system_prompt = ADDITIVE_EXTRACTION_PROMPT
            if is_agent_scoped:
                system_prompt += AGENT_CONTEXT_SUFFIX
            system_prompt += IMPORT_EXTRACTION_SUFFIX
            system_prompt += _import_core_evidence_prompt(
                core_source_indices,
                [message["source_index"] for message in source_messages],
                has_overlap,
            )
            user_prompt = generate_additive_extraction_prompt(
                existing_memories=existing_memories,
                new_messages=source_messages,
                last_k_messages=last_messages,
            ...
            with timer.track("llm"):
                extracted_memories = self._extract_import_memories(
                    llm_messages,
                    source_messages,
                    operation_context,
                )
            ...
            for extracted_memory in extracted_memories:
                ...
                if operation_context.memory_hash_claim is not None:
                    _run_import_execution_guard(operation_context)
                    claim_result = operation_context.memory_hash_claim(memory_hash)
                    if not isinstance(claim_result, bool):
                        raise TypeError("memory_hash_claim must return bool")
                    if not claim_result:
                        continue
                    operation_context.claimed_memory_hashes.append(memory_hash)
                selected_memories.append((extracted_memory, memory_hash))

            if not selected_memories:
                self._save_import_messages(source_messages, metadata, session_scope, operation_context)
                return []

            if operation_context.pre_vector_write is not None:
                operation_context.pre_vector_write(len(selected_memories))

            memory_texts = [memory["text"] for memory, _memory_hash in selected_memories]
            batch_embedding_error = None
            with timer.track("embedding"):
                try:
                    memory_embeddings = list(self.embedding_model.embed_batch(memory_texts, "add"))
                except Exception as exc:
                    batch_embedding_error = exc
                    memory_embeddings = []
                    for memory_text in memory_texts:
                        try:
                            memory_embeddings.append(self.embedding_model.embed(memory_text, "add"))
                        except Exception as item_error:
                            batch_embedding_error = item_error
                            memory_embeddings.append(None)
            if len(memory_embeddings) != len(memory_texts) or any(embedding is None for embedding in memory_embeddings):
                raise EmbeddingError("Import memory embedding failed.") from batch_embedding_error
            ...
            records = []
            generated_ids = set()
            for (extracted_memory, memory_hash), embedding in zip(selected_memories, memory_embeddings):
                if operation_context.id_factory is None:
                    memory_id = str(uuid.uuid4())
                else:
                    memory_id = operation_context.id_factory(memory_hash)
                    if not isinstance(memory_id, str) or not memory_id.strip():
                        raise TypeError("id_factory must return a non-empty string")
                    memory_id = memory_id.strip()
                if memory_id in generated_ids:
                    raise ValueError("id_factory returned duplicate memory ids")
                generated_ids.add(memory_id)

                memory_metadata = deepcopy(metadata)
                memory_metadata.pop("_allowed_categories", None)
                memory_metadata.update(
                    {
                        "data": extracted_memory["text"],
                        "text_lemmatized": lemmatize_for_bm25(extracted_memory["text"]),
                        "hash": memory_hash,
                        "source_message_indices": list(extracted_memory["source_message_indices"]),
                        "confidence": extracted_memory["confidence"],
                    }
                )
                if "created_at" not in memory_metadata:
                    memory_metadata["created_at"] = datetime.now(timezone.utc).isoformat()
                memory_metadata["updated_at"] = memory_metadata["created_at"]
                if extracted_memory.get("attributed_to"):
                    memory_metadata["attributed_to"] = extracted_memory["attributed_to"]
                linked_ids = extracted_memory.get("linked_memory_ids") or []
                if linked_ids:
                    memory_metadata["linked_memory_ids"] = [uuid_mapping.get(value, value) for value in linked_ids]
                categories = _normalize_memory_categories(
                    extracted_memory.get("categories") or extracted_memory.get("category"),
                    allowed_categories,
                )
                if categories:
                    memory_metadata["categories"] = categories
                records.append((memory_id, extracted_memory["text"], embedding, memory_metadata))
            ...
            with timer.track("pgvector"):
                try:
                    ...
                    if operation_context.strict_vector_write:
                        raise
                    persisted_records = []
                    for record in records:
                        try:
                            ...
                            logger.error("Import memory vector insert failed for id=%s", record[0])
            records = persisted_records
            if not records:
                self._save_import_messages(source_messages, metadata, session_scope, operation_context)
                return []

            self._complete_import_side_effects(
                records,
                source_messages,
                filters,
                metadata,
            ...
            returned_memories = [_format_added_memory(record) for record in records]
            keys, encoded_ids = process_telemetry_filters(filters)
            capture_event(
                "mem0.add",
                self,
    ...
    def delete_all(
        ...
        Args:
            ...
            app_id (str, optional): ID of the app to delete memories for. Defaults to None.
            project_id (str, optional): Project scope to delete memories from. Defaults to None.
        """
        user_id = _validate_and_trim_entity_id(user_id, "user_id")
        agent_id = _validate_and_trim_entity_id(agent_id, "agent_id")
        run_id = _validate_and_trim_entity_id(run_id, "run_id")
    ...
    def _create_memory(self, data, existing_embeddings, metadata=None):
        ...
        self.db.add_history(
            ...
            project_id=new_metadata.get("project_id"),
        )
        return memory_id

    def _create_procedural_memory(self, messages, metadata=None, prompt=None, pre_vector_write=None):
        """
...
class AsyncMemory(MemoryBase):
    ...
    async def delete_all(self, user_id=None, agent_id=None, run_id=None, app_id=None, project_id=None):
        """

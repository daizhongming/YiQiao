...
def test_bad_or_disabled_payload_is_silent_and_safe(notice_harness, payload, expected_reason, expected_found, capsys):
...
def test_temporal_feature_blunt_flag_disable_returns_plain_error_without_event(notice_harness, variant, capsys):
    _, telemetry = notice_harness
...
def test_async_temporal_feature_wrapper_uses_shared_helper(monkeypatch):
    ...
    message = asyncio.run(notices.get_temporal_feature_error_message_async("async", "search", "reference_date"))
...
@pytest.mark.parametrize("variant", [None, False])
def test_decay_feature_blunt_flag_disable_returns_plain_error_without_event(notice_harness, variant, capsys):
    _, telemetry = notice_harness
...
def test_async_decay_feature_wrapper_uses_shared_helper(monkeypatch):
    ...
    message = asyncio.run(notices.get_decay_feature_error_message_async("async", "project.update", "decay"))
...
@pytest.mark.parametrize("variant", [None, False])
def test_temporal_usage_blunt_flag_disable_does_not_capture_or_consume(notice_harness, variant, capsys):
    config, telemetry = notice_harness
...
@pytest.mark.parametrize("variant", [None, False])
def test_decay_usage_blunt_flag_disable_does_not_capture_or_consume(notice_harness, variant, capsys):
    config, telemetry = notice_harness
...
def test_decay_usage_cap_ignores_old_entries(notice_harness, capsys):
    ...
    config["notice_state"] = {
        "decay_usage": {
            "events": [
                {"evaluated_at": old_time.isoformat(), "variant": "displayed"} for _ in range(notices.DECAY_USAGE_CAP)
            ]
        }
    }
    configure_flag(telemetry, "displayed", decay_usage_payload())
...
@pytest.mark.parametrize("variant", [None, False])
def test_scale_threshold_blunt_flag_disable_does_not_capture_or_consume(notice_harness, variant, capsys):
    config, telemetry = notice_harness

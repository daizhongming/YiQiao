...
def configured_telemetry_key():
    with patch.object(telemetry_module, "PROJECT_API_KEY", "test-project-key"):
        yield
...
class TestTelemetryDisabled:
    ...
    def test_posthog_not_created_without_operator_key(self):
        with (
            patch.object(telemetry_module, "MEM0_TELEMETRY", True),
            patch.object(telemetry_module, "PROJECT_API_KEY", ""),
            patch("mem0.memory.telemetry.Posthog") as mock_posthog,
        ):
            telemetry = telemetry_module.AnonymousTelemetry()

        mock_posthog.assert_not_called()
        assert telemetry.posthog is None
...
class TestTelemetryEnabled:
        ...
        with patch.object(telemetry_module, "MEM0_TELEMETRY", True):
            with patch("mem0.memory.telemetry.Posthog") as mock_posthog:
                with patch("mem0.memory.telemetry.get_or_create_user_id", return_value="test-user"):
                    ...
                    assert mock_posthog.call_args.kwargs["disable_geoip"] is True
    ...
    def test_capture_event_excludes_identifiers_and_collection_name(self):
        with patch.object(telemetry_module, "MEM0_TELEMETRY", True):
            mock_at = MagicMock()
            mock_memory = MagicMock()
            mock_memory.collection_name = "customer-private-collection"
            mock_memory.api_version = "v1"
            with patch.object(telemetry_module, "_oss_telemetry_instance", mock_at):
                telemetry_module.capture_event(
                    "test.event",
                    mock_memory,
                    {
                        "memory_id": "private-memory-id",
                        "encoded_ids": {"user_id": "stable-user-hash"},
                        "sync_type": "sync",
                    },
                )

        properties = mock_at.capture_event.call_args.args[1]
        assert properties["sync_type"] == "sync"
        assert "collection" not in properties
        assert "memory_id" not in properties
        assert "encoded_ids" not in properties
...
class TestTelemetryEnvVar:
    """Verify YiQiao's fail-closed opt-in parsing and alias precedence."""
    ...
    def test_env_var_parsing(self, value, expected):
        assert telemetry_module._parse_bool(value) is expected

    def test_default_is_disabled(self):
        with patch.dict(os.environ, {}, clear=True):
            assert telemetry_module._telemetry_requested() is False

    def test_legacy_true_is_still_an_explicit_opt_in(self):
        with patch.dict(os.environ, {"MEM0_TELEMETRY": "true"}, clear=True):
            assert telemetry_module._telemetry_requested() is True

    def test_yiqiao_setting_takes_precedence_over_legacy_alias(self):
        with patch.dict(
            os.environ,
            {"YIQIAO_TELEMETRY": "false", "MEM0_TELEMETRY": "true"},
            clear=True,
        ):
            assert telemetry_module._telemetry_requested() is False

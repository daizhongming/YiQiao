...
def test_default_fields_memory_is_text():
    ...
    assert memory_field["type"] == "text", "DEFAULT_FIELDS['memory'] must be 'text' (full-text searchable), not 'tag'"
...
def test_build_index_schema_indexes_memory_as_text(valkey_db):
    ...
    assert cmd[memory_idx + 1] == "TEXT", f"'memory' field must be indexed as TEXT, got {cmd[memory_idx + 1]!r}"

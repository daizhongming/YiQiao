...
## Import Traceability Contract

This request is part of a chat-history import. The New Messages are structured objects with
`role`, `content`, and `source_index`. The following rules override any examples above that omit
traceability fields:

- Every memory object MUST include `source_message_indices`, a non-empty array of integer
  `source_index` values copied exactly from the New Messages that support the entire memory.
- Every memory object MUST include `confidence`, a JSON number from 0.0 through 1.0 expressing
  confidence that the cited source messages directly support the complete memory text.
- Cite only New Messages. Never cite Last k Messages, Existing Memories, summaries, or an index
  that is not present in New Messages.
- Split a memory when its claims require different evidence. Do not attach an unrelated source
  merely to satisfy the schema.
- Do not extract claims with missing evidence. If cited messages conflict and the conflict cannot
  be resolved from New Messages, omit that claim. If a candidate is emitted despite an unresolved
  conflict, set `conflict` to true so the caller can reject it and route to a stronger model.
- Preserve `source_message_indices` and `confidence` on every item even when the memory list has
  only one item.
...

{
  "memory": [
    {
      "id": "0",
      "text": "A self-contained memory supported by the cited messages",
      "attributed_to": "user",
      "source_message_indices": [12],
      "confidence": 0.95
    }
  ]
}

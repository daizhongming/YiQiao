...
def test_embed_forwards_allowed_openai_params_for_compatible_proxy(mock_openai_client):
    config = BaseEmbedderConfig(
    ...
    embedder = OpenAIEmbedding(config)
    mock_openai_client.embeddings.create.return_value = Mock(data=[Mock(embedding=[0.1] * 1024)])

    embedder.embed("中文嵌入测试")
    ...
    mock_openai_client.embeddings.create.assert_called_once_with(
        ...
        extra_body={"allowed_openai_params": ["dimensions"]},
    )


def test_embed_batch_forwards_allowed_openai_params_for_compatible_proxy(mock_openai_client):
    config = BaseEmbedderConfig(
        model="zhipu/embedding-3",
        embedding_dims=1024,
        allowed_openai_params=["dimensions"],
    ...
    mock_openai_client.embeddings.create.return_value = Mock(
        data=[Mock(index=0, embedding=[0.1] * 1024), Mock(index=1, embedding=[0.2] * 1024)]
    )

    embedder.embed_batch(["第一条记忆", "第二条记忆"])

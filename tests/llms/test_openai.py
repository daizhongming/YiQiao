...
def test_openai_client_timeout_and_retries_forwarded():
    with patch("mem0.llms.openai.OpenAI") as mock_openai:
        config = OpenAIConfig(
            model="gpt-4.1-nano-2025-04-14",
            api_key="api_key",
...
def test_generate_response_rejects_truncated_completion(mock_openai_client):
    config = OpenAIConfig(model="gpt-4.1-nano-2025-04-14", max_tokens=100)
    llm = OpenAILLM(config)
    mock_response = Mock()
    mock_response.choices = [Mock(message=Mock(content='{"memory": ['), finish_reason="length")]
    mock_openai_client.chat.completions.create.return_value = mock_response

    with pytest.raises(LLMError, match="truncated.*100 tokens"):
        llm.generate_response([{"role": "user", "content": "remember this"}])

...
4. Enforce the required checks on protected `main`, prohibit force-pushes and
   deletion, then create `v1.0.0` at that exact commit. Verify the tag resolves
   to the reviewed `main` SHA.
5. Dispatch `YiQiao Images` from `main` with `publish=true` and
   `version=v1.0.0`. The workflow must reject a version tag that is missing or
   targets another commit.
6. Verify both GHCR packages are public and anonymously resolve `latest`,
   `v1.0.0`, and full commit-SHA tags. Record the immutable multi-platform
   digest for each image.
7. Create the bilingual GitHub Release for `v1.0.0`, including the exact source
...
- Version `1.0.x` is the first stable compatibility line; operators should review
  API, schema, provider, and image compatibility before major upgrades.
